...
FLUTTER_DEFENDER_EXPORT int32_t fd_is_debugger_attached() {
  return flutter_defender::IsDebuggerAttached() ? 1 : 0;
}

FLUTTER_DEFENDER_EXPORT int32_t fd_is_rooted_or_jailbroken() {
  return flutter_defender::IsRootedOrJailbroken() ? 1 : 0;
}

FLUTTER_DEFENDER_EXPORT int32_t fd_is_emulator() {
  return flutter_defender::IsEmulator() ? 1 : 0;
}

FLUTTER_DEFENDER_EXPORT int32_t fd_is_tampered() {
  return flutter_defender::IsTampered() ? 1 : 0;
}

FLUTTER_DEFENDER_EXPORT char* fd_hmac_sha256_hex(
    const uint8_t* data,
    int32_t data_length,
    const uint8_t* key,
    int32_t key_length) {
...
FLUTTER_DEFENDER_EXPORT void fd_free_string(char* value) {
  std::free(value);
}
